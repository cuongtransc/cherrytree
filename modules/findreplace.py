...
'idialog':True}
    ...
    def find_a_node(self):
        ...
        while node_iter != None:
            ...
            if self.replace_active:
                if self.parse_node_name(node_iter, pattern, forward,
                                        all_matches):
                    self.matches_num += 1
                ...
                if self.parse_node_name_fuzzy(node_iter, pattern, forward,
                                               all_matches):
                    self.matches_num += 1
    ...
    def parse_node_name_fuzzy(self, node_iter, pattern, forward, all_matches):
        """Recursive function that searchs for the given pattern"""
        if self.is_node_within_time_filter(node_iter):
            text_name = self.dad.treestore[node_iter][1].decode(cons.STR_UTF8)
            ...
            if not match:
                match = fuzz.token_sort_ratio(pattern.pattern.lower(),
                                              text_name.lower()) > 70
